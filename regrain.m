...
%
%   IRR = degrain(I_original, I_colourgraded);
%
%  (c) F. Pitie 2007
%
%  see reference:
...
%
function IRR = regrain(I0, IR)

IRR = I0;
[IRR] = regrain_rec(IRR, I0, IR, [4 4 16 32 64 128 128]);

end

function [IRR] = solve(IRR, I0, IR, nbits)
...
G0 = blur(I0, 5);
G0x = (G0(:,[2:end end], :) - G0(:,1:end, :))*255;
G0y = (G0([2:end end], :, :) - G0(1:end, :, :))*255;
...
function [IRR] = regrain_rec(IRR, I0, IR, nbits)
...
if (length(nbits) > 1 && hres2 > 20 && vres2 > 20)
    ...
    [IRR2] = regrain_rec(IRR2, I02, IR2, nbits(2:end));
    IRR = imresize(IRR2, [vres hres], 'bilinear');
end

IRR = solve(IRR, I0, IR, nbits(1));
